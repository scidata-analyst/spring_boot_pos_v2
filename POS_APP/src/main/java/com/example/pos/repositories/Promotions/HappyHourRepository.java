...
public interface HappyHourRepository extends JpaRepository<HappyHour, Long> {
     ...
     */
    default Page<HappyHour> index(String search, Pageable pageable) {
    ...
    @Query("SELECT e FROM HappyHour e WHERE "
           + "LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.daysOfWeek) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.status) LIKE LOWER(CONCAT('%', :search, '%'))")
    Page<HappyHour> searchAll(@Param("search") String search, Pageable pageable);
