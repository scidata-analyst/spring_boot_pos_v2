package com.example.pos.repositories.Promos;

import com.example.pos.entities.Promos.HappyHour;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import java.util.List;
import java.util.Optional;

/**
 * Repository interface for HappyHour entity.
 * <p>
 * Provides full CRUD operations for HappyHour using JpaRepository.
 * Method names are aligned with Controller and Service:
 * getAll(), get(Long id), create(), update(), delete().
 * JpaRepository handles create, update, and delete by default.
 * Custom queries can be added here following the same naming convention.
 */
@Repository
public interface HappyHourRepository extends JpaRepository<HappyHour, Long> {

    /**
     * Retrieve all HappyHour records.
     * Corresponds to getAll() in Controller/Service.
     * @return List of HappyHour
     */
    default List<HappyHour> getAll() {
        return findAll();
    }

    /**
     * Retrieve a single HappyHour by its ID.
     * Corresponds to get(Long id) in Controller/Service.
     * @param id ID of the HappyHour
     * @return Optional containing HappyHour if found
     */
    default Optional<HappyHour> get(Long id) {
        return findById(id);
    }

    /**
     * Save or update a HappyHour.
     * Corresponds to create() or update() in Controller/Service.
     * @param entity Entity object to save
     * @return Saved HappyHour
     */
    default HappyHour saveEntity(HappyHour entity) {
        return save(entity);
    }

    /**
     * Delete a HappyHour by ID.
     * Corresponds to delete() in Controller/Service.
     * @param id ID of the entity to delete
     */
    default void deleteEntity(Long id) {
        deleteById(id);
    }

    // TODO: Add more custom query methods as needed
}
