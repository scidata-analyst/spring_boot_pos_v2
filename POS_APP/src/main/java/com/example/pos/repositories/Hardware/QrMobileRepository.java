package com.example.pos.repositories.Hardware;

import com.example.pos.entities.Hardware.QrMobile;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import java.util.List;
import java.util.Optional;

/**
 * Repository interface for QrMobile entity.
 * <p>
 * Provides full CRUD operations for QrMobile using JpaRepository.
 * Method names are aligned with Controller and Service:
 * getAll(), get(Long id), create(), update(), delete().
 * JpaRepository handles create, update, and delete by default.
 * Custom queries can be added here following the same naming convention.
 */
@Repository
public interface QrMobileRepository extends JpaRepository<QrMobile, Long> {

    /**
     * Retrieve all QrMobile records.
     * Corresponds to getAll() in Controller/Service.
     * @return List of QrMobile
     */
    default List<QrMobile> getAll() {
        return findAll();
    }

    /**
     * Retrieve a single QrMobile by its ID.
     * Corresponds to get(Long id) in Controller/Service.
     * @param id ID of the QrMobile
     * @return Optional containing QrMobile if found
     */
    default Optional<QrMobile> get(Long id) {
        return findById(id);
    }

    /**
     * Save or update a QrMobile.
     * Corresponds to create() or update() in Controller/Service.
     * @param entity Entity object to save
     * @return Saved QrMobile
     */
    default QrMobile saveEntity(QrMobile entity) {
        return save(entity);
    }

    /**
     * Delete a QrMobile by ID.
     * Corresponds to delete() in Controller/Service.
     * @param id ID of the entity to delete
     */
    default void deleteEntity(Long id) {
        deleteById(id);
    }

    // TODO: Add more custom query methods as needed
}
